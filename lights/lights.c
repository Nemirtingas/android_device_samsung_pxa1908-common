...
#define BOARD_PANEL_BRIGHTNESS "/sys/class/backlight/panel/brightness"
            ...
            struct light_state_t const *state)
{
    ...
    err = write_int(BOARD_PANEL_BRIGHTNESS, brightness);
...
set_light_buttons(UNUSED struct light_device_t* dev,
        struct light_state_t const* state)
{
#ifdef BOARD_BUTTON_BRIGHTNESS
    int err = 0;
    int on = is_lit(state);

    pthread_mutex_lock(&g_lock);
    err = write_int(BOARD_BUTTON_BRIGHTNESS, on?1:0);
    pthread_mutex_unlock(&g_lock);
...
struct hw_module_t HAL_MODULE_INFO_SYM = {
    ...
    .name = "GrandPrimeVELTE Lights Module",
